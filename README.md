...
├── LICENSE
├── analysis/
│   ├── syscheck_anomaly_detection.py   # Isolation Forest over Wazuh syscheck events
│   └── requirements.txt
└── docs/
...
python syscheck_anomaly_detection.py
```

Expects a `syscheck.json` export from the Wazuh API/indexer (the `hits.hits` search-result shape) in the same folder, and writes scored events to `syscheck_anomaly_results.csv`.
